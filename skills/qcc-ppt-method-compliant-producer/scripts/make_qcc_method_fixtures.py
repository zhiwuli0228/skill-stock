...
def build_data_complete(path: Path) -> None:
    ...
    add_text_slide(
        prs,
        "现状把握｜现状流程图",
        ["开始 → 接收任务 → 执行处理 → 数据记录 → 判定：结果校验是否通过？",
         "判定为「通过」→ 结束；判定为「不通过」→ 异常分支 → 结果校验异常（异常点）。",
         "流程路径：开始 → 接收任务 → 执行处理 → 数据记录 → 判定 → 不通过 → 异常点 → 结束。",
         "异常点：结果校验环节异常率最高（42%）。"],
    )
    ...
    add_table_slide(
        prs,
        "解析｜要因评价",
        ["候选要因", "影响度", "发生频次", "可控性", "总分", "筛选结论"],
        [
            ["校验规则缺失", "5", "5", "5", "15", "进入真因验证"],
            ["字段定义不统一", "4", "4", "4", "12", "进入真因验证"],
            ["抽样标准不明确", "3", "3", "3", "9", "本轮不验证"],
        ],
        ["评分口径：各维度 5 分制，总分 15 分；≥10 分进入真因验证。",
         "总分排序后筛选出要因：校验规则缺失、字段定义不统一（抽样标准不明确本轮不验证）。"],
    )
    ...
    add_table_slide(
        prs,
        "对策实施与检讨",
        ["对策", "实施时间", "责任人", "实施内容", "过程数据", "困难与调整"],
        [
            ["补齐校验规则", "第4周", "李四", "新增 12 条校验规则并上线", "异常率 42.0% → 18.0%",
             "工具兼容性问题，延期 2 天"],
            ["统一字段定义", "第4周", "王五", "发布字段字典并培训", "字段口径差异清零",
             "培训与生产高峰冲突"],
        ],
        ["阶段：D 实施与 C 检查。进展：两条对策全部完成，完成率 100%。",
    ...
    add_text_slide(
        ...
        [
            ...
            "改善前后对比图（柏拉图）：异常率显著下降。",
            "统计检验：χ² 检验，p < 0.05，差异显著。",
            "效益核算：投入 24 人时（约 0.4 万元），年化节省 312 小时（约 5.2 万元），回收期 < 1 个月。",
        ],
    )
