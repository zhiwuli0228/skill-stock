...
def is_badge_shape(shape, include_rounded: bool = False) -> bool:
    try:
        kind = str(shape.auto_shape_type)
    except Exception:
        return False
    if "OVAL" in kind:
        return True
    return include_rounded and "ROUNDED_RECTANGLE" in kind
...
def center_badges(pptx_path: Path, output_path: Path, include_rounded: bool = False) -> Counter:
    prs = Presentation(str(pptx_path))
    changes: Counter = Counter()

    for slide_idx, slide in enumerate(prs.slides, start=1):
        ovals = [
            shape for shape in slide.shapes if is_badge_shape(shape, include_rounded)
        ]
        candidates = []
...
def main() -> None:
    ...
    parser.add_argument(
        "--include-rounded",
        action="store_true",
        help="also treat rounded rectangles as badge backgrounds",
    )
    args = parser.parse_args()

    changes = center_badges(args.pptx, args.output, include_rounded=args.include_rounded)
